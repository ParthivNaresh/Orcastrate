...
class MongoDBTool(DatabaseTool):
    ...
    def _create_db_config(self) -> DatabaseConfig:
        ...
        return DatabaseConfig(
            ...
            port=env.get("port", 27017),
            database=env.get("database", "test"),
            username=env.get("username", ""),
            password=env.get("password", ""),
            min_connections=env.get("min_connections", 1),
            max_connections=env.get("max_connections", 10),
            connection_timeout=env.get("connection_timeout", 30.0),
            idle_timeout=env.get("idle_timeout", 300.0),
            max_lifetime=env.get("max_lifetime", 3600.0),
